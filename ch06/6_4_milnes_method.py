...
def predictor(x_vals: list[float], y_vals: list[float], h: float) -> float:
    f_vals = 2.*f(x_vals[3], y_vals[3]) - f(x_vals[2], y_vals[2]) + 2.*f(x_vals[1], y_vals[1])
...
def main():
    ...
    for i in range(2):
        predicted = predictor(x_vals[i:i+4], output_vals[i:i+4], h)
        print(f"y_{i+4},p = {predicted}")
        output_vals.append(predicted)
        corrected = corrector(x_vals[i+2:i+5], output_vals[i+2:i+5], h)
